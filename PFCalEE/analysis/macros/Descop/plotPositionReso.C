...
struct Result{
  ...
  double residual_x_rmserr;
  double residual_y_rmserr;
  double residual_tanx_rmserr;
  double residual_tany_rmserr;
};


Result plotOnePoint(unsigned version, unsigned etabin, unsigned pt, unsigned pu){//main
  ...
  res.residual_x_rmserr = 0;
  res.residual_y_rmserr = 0;
  res.residual_tanx_rmserr = 0;
  res.residual_tany_rmserr = 0;

  std::ostringstream plotDir;
  //plotDir << "/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalDescopPos/gitV05-02-04/version" << version << "/gamma/fixedVtx_/eta";
  plotDir << "/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalDescopPosCheck/gitV02-01-01/version" << version << "/gamma/v5_30_/eta";

  if (etabin<19) plotDir << etabin;
  else plotDir << etabin+2;
  plotDir << "_et" << pt << "_pu" << pu << "_IC3";
  ...
  TH1F *p_impactX = (TH1F*)gDirectory->Get("p_impactX14");
  TH1F *p_impactX_residual = (TH1F*)gDirectory->Get("p_impactX14_residual");
  TH1F *p_impactX_truth = (TH1F*)gDirectory->Get("p_impactX14_truth");
  ...
  TH1F *p_impactY = (TH1F*)gDirectory->Get("p_impactY14");
  TH1F *p_impactY_residual = (TH1F*)gDirectory->Get("p_impactY14_residual");
  TH1F *p_impactY_truth = (TH1F*)gDirectory->Get("p_impactY14_truth");
...
int plotPositionReso(){//main
  ...
  const unsigned eta[neta] = {16};//7,19,21,23,25,27};
  ...
  for (unsigned ipt=0; ipt<(runAll?npt:1);++ipt){//loop on pt values
	...
	Result lres = plotOnePoint(version,eta[ie],pt[ipt],pu[ipu]);
	if (lres.nEvts > 0){
	...
	if (!runAll) {
      ...
      double residual_x_rmserr[nP];
      double residual_y_rmserr[nP];
	...
	impactX_rms[iP] = fabs(resVec[iP].impactX_rms);
	impactY_rms[iP] = fabs(resVec[iP].impactY_rms);
	tanAngleX_rms[iP] = fabs(resVec[iP].tanAngleX_rms);
	tanAngleY_rms[iP] = fabs(resVec[iP].tanAngleY_rms);
	...
	residual_x_rms[iP] = fabs(resVec[iP].residual_x_rms);
	residual_y_rms[iP] = fabs(resVec[iP].residual_y_rms);
	residual_tanx_rms[iP] = fabs(resVec[iP].residual_tanx_rms*1000);
	residual_tany_rms[iP] = fabs(resVec[iP].residual_tany_rms*1000);
	residual_x_rmserr[iP] = fabs(resVec[iP].residual_x_rmserr);
	residual_y_rmserr[iP] = fabs(resVec[iP].residual_y_rmserr);
	residual_tanx_rmserr[iP] = fabs(resVec[iP].residual_tanx_rmserr*1000);
	residual_tany_rmserr[iP] = fabs(resVec[iP].residual_tany_rmserr*1000);
	...
	grResidualX[ipt][ipu] = new TGraphErrors(nP,etaval[ipu],residual_x_rms,etaerr,residual_x_rmserr);
	grResidualY[ipt][ipu] = new TGraphErrors(nP,etaval[ipu],residual_y_rms,etaerr,residual_y_rmserr);
	grResidualTanX[ipt][ipu] = new TGraphErrors(nP,etaval[ipu],residual_tanx_rms,etaerr,residual_tanx_rmserr);
	grResidualTanY[ipt][ipu] = new TGraphErrors(nP,etaval[ipu],residual_tany_rms,etaerr,residual_tany_rmserr);
      }
      ...
      grEvts[ipt][ipu]->SetMaximum(5200);
      ...
      grResidualX[ipt][ipu]->SetMaximum(doMean?4:1);//max_pos);
      grResidualX[ipt][ipu]->SetMinimum(doMean?-4:0);//max_pos);
      ...
      grResidualTanX[ipt][ipu]->SetMaximum(doMean?20:10);//max_angle);
      grResidualTanX[ipt][ipu]->SetMinimum(doMean?-20:0);//max_angle);
  ...
  lPrint.str("");
  lPrint << "PLOTS_v" << version << "/Summary_nEvts";
  ...
  lPrint.str("");
  lPrint << "PLOTS_v" << version << "/Summary_fitQuality";
  if (!runAll) lPrint << "_et" << pt[0];
  lPrint << ".pdf";
  mycFit->Update();
  ...
  lPrint.str("");
  lPrint << "PLOTS_v" << version << "/Summary_ResidualPos";
  if (!doMean) lPrint << "RMS";
  if (!runAll) lPrint << "_et" << pt[0];
  lPrint << ".pdf";  
  mycRP->Update();
  ...
  lPrint.str("");
  lPrint << "PLOTS_v" << version << "/Summary_ResidualAngle";
  if (!doMean) lPrint << "RMS";
  if (!runAll) lPrint << "_et" << pt[0];
  lPrint << ".pdf";
  mycRA->Update();
